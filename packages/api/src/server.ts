...
import {
  ...
  recentMemory as npcMemory, rejectPersona, relations as npcRelations, intentions as npcIntentions, setAgentEnabled,
} from './world/Npc.ts';
import {
...
app.get('/admin/npc/:id/intentions', ...staff, async (req: AuthedRequest, res, next) => {
  try { res.json({ intentions: await npcIntentions(param(req.params.id), npcLimit(req)) }); } catch (err) { next(err); }
});
